...
-- Description: Updates check_geofence and check_early_departure to return TABLE structures

BEGIN;

-- 1. Redefine check_geofence to return TABLE (matching EnhancedAttendanceService)
DROP FUNCTION IF EXISTS check_geofence(UUID, NUMERIC, NUMERIC);
DROP FUNCTION IF EXISTS check_geofence(DOUBLE PRECISION, DOUBLE PRECISION, UUID);
...
-- 2. Redefine check_early_departure to return TABLE (matching attendance.routes.ts)
DROP FUNCTION IF EXISTS check_early_departure(UUID, TIMESTAMPTZ);

CREATE OR REPLACE FUNCTION check_early_departure(
    p_company_id UUID,
