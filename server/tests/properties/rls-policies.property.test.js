...
const uuidArbitrary = fast_check_1.default.uuid();
async function createTestCompany(companyId, name) {
    const slug = `${name.toLowerCase().replace(/\s+/g, '-')}-${companyId.substring(0, 8)}`;
    await pool.query(`INSERT INTO companies (id, name, email, slug, employee_count, created_at)
     VALUES ($1, $2, $3, $4, $5, NOW())
     ON CONFLICT (id) DO NOTHING`, [companyId, name, `${slug}@test.com`, slug, 10]);
}
/**
 * Helper: Create a test user
 */
async function createTestUser(userId, companyId, email, role) {
    await pool.query(`INSERT INTO users (id, company_id, email, password_hash, first_name, last_name, role, email_verified, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, true, NOW())
     ON CONFLICT (id) DO NOTHING`, [userId, companyId, email, 'test_hash', 'Test', 'User', role]);
}
/**
 * Helper: Set PostgreSQL session variables for RLS
 */
async function setSessionContext(tenantId, userId) {
 ...
 */
async function createCompanySettings(companyId) {
    const result = await pool.query(`INSERT INTO company_settings (company_id)
     VALUES ($1)
 ...
 */
async function attemptReadCompanySettings(companyId) {
    const result = await pool.query(`SELECT * FROM company_settings WHERE company_id = $1`, [companyId]);
    return result.rows;
}
/**
 * Helper: Attempt to read notifications (should be blocked by RLS if cross-tenant or cross-user)
 */
async function attemptReadNotifications(userId) {
 ...
 */
async function createNotification(companyId, userId, type, title) {
    const result = await pool.query(`INSERT INTO notifications (company_id, user_id, type, title, body, created_at)
     VALUES ($1, $2, $3, $4, $5, NOW())
     RETURNING id`, [companyId, userId, type, title, 'Test notification']);
    return result.rows[0].id;
}
async function attemptReadAuditLogs(companyId) {
    const result = await pool.query(`SELECT * FROM audit_logs WHERE company_id = $1`, [companyId]);
    return result.rows;
}
/**
 * Helper: Create an audit log entry
 */
async function createAuditLog(companyId, userId, action) {
    const result = await pool.query(`INSERT INTO audit_logs (company_id, user_id, action, entity_type, entity_id, created_at)
     VALUES ($1, $2, $3, $4, $5, NOW())
     RETURNING id`, [companyId, userId, action, 'test_entity', companyId]);
    return result.rows[0].id;
}
(0, globals_1.describe)('Property 6: Cross-Tenant Access Prevention', () => {
    ...
    (0, globals_1.it)('should block all cross-tenant access to company_settings', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // userTenantId
        ...
        async (userTenantId, resourceCompanyId, userId) => {
            ...
            fast_check_1.default.pre(userTenantId !== resourceCompanyId);
            try {
    ...
    (0, globals_1.it)('should block all cross-tenant access to notifications', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // userTenantId
        uuidArbitrary, // resourceCompanyId
        uuidArbitrary, // userId
        ...
        async (userTenantId, resourceCompanyId, userId, targetUserId) => {
            ...
            fast_check_1.default.pre(userTenantId !== resourceCompanyId && userId !== targetUserId);
            try {
                // Setup: Create two companies
                await createTestCompany(userTenantId, `Company ${userTenantId.substring(0, 8)}`);
                await createTestCompany(resourceCompanyId, `Company ${resourceCompanyId.substring(0, 8)}`);
    ...
    (0, globals_1.it)('should block cross-user access to notifications within same tenant', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // tenantId
        uuidArbitrary, // userId1
        uuidArbitrary, // userId2
        async (tenantId, userId1, userId2) => {
            // Precondition: Different users in same tenant
            fast_check_1.default.pre(userId1 !== userId2);
            try {
    ...
    (0, globals_1.it)('should allow same-tenant access to company_settings', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // tenantId
        uuidArbitrary, // userId
        async (tenantId, userId) => {
            try {
                ...
                (0, globals_1.expect)(results[0].company_id).toBe(tenantId);
                // Reset session context
                await resetSessionContext();
            }
            catch (error) {
    ...
    (0, globals_1.it)('should allow same-user access to notifications', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // tenantId
        uuidArbitrary, // userId
        async (tenantId, userId) => {
            try {
                ...
                (0, globals_1.expect)(results.length).toBeGreaterThan(0);
                (0, globals_1.expect)(results[0].user_id).toBe(userId);
                (0, globals_1.expect)(results[0].company_id).toBe(tenantId);
                // Reset session context
                await resetSessionContext();
            }
    ...
    (0, globals_1.it)('should block all cross-tenant access to audit_logs', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // userTenantId
        uuidArbitrary, // resourceCompanyId
        uuidArbitrary, // userId
        async (userTenantId, resourceCompanyId, userId) => {
    ...
    (0, globals_1.it)('should allow same-tenant access to audit_logs (read-only)', async () => {
        await fast_check_1.default.assert(fast_check_1.default.asyncProperty(uuidArbitrary, // tenantId
        uuidArbitrary, // userId
        async (tenantId, userId) => {
            try {
...
//# sourceMappingURL=rls-policies.property.test.js.map