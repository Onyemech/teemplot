...
import { Coordinates } from '../../src/utils/geolocation';
...
describe('AttendanceService', () => {
  ...
  const officeLocation: Coordinates = { latitude: 40.7128, longitude: -74.0060 }; // NYC
  const farLocation: Coordinates = { latitude: 34.0522, longitude: -118.2437 }; // LA

  beforeEach(() => {
    jest.clearAllMocks();
    mockQuery.mockReset();
    // Setup transaction mock implementation
    (transaction as jest.Mock).mockImplementation((callback) => callback(mockClient));
  });
