...
export class GoogleAuthService {
    ...
    try {
      ...
      if (existingUser) {
        ...
        return {
          ...
          isNewUser: false,
          requiresOnboarding: !existingUser.company_id || existingUser.company_id === 'temp'
        };
      }

      // New user - create temporary user record for onboarding
      const userId = randomUUID();
      const tempCompanyId = 'temp'; // Temporary until onboarding completes
  ...
  async completeOnboarding(userId: string, companyData: {
    ...
    try {
      ...
      await this.db.insert('companies', company);

      // Update user with real company ID
      await this.db.update(
        'users',
        { 
      ...
      await this.syncCompanyToSupabase(company);
      ...
      logger.info({ userId, companyId }, 'Google user onboarding completed and synced to Supabase');
