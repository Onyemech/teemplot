...
export default async function leaveRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/request', {
    ...
    try {
      ...
      const normalizedLeaveType = TYPE_MAP[String(leaveType).trim().toLowerCase()] || null;
      ...
      if (normalizedLeaveType === 'annual' && balance < totalDays) {
        return reply.code(400).send({
          success: false,
      ...
      const result = await db.query(
        ...
        RETURNING *`,
        [companyId, userId, normalizedLeaveType, startDate, endDate, totalDays, reason, halfDay || false, initialStage]
      );
      ...
      await db.query(
        ...
        [companyId, userId, leaveRequest.id, JSON.stringify({ leaveType: normalizedLeaveType, totalDays })]
      );
