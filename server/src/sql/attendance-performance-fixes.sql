...
-- 2. Optimize the getCompanyAttendance function with a materialized view for dashboard stats
CREATE MATERIALIZED VIEW mv_attendance_dashboard AS
SELECT 
    ar.company_id,
...
CREATE INDEX idx_mv_attendance_dashboard_company_date 
ON mv_attendance_dashboard(company_id, attendance_date DESC);

-- 3. Create function for optimized geofence checking
...
-- 4. Add row-level locking function for concurrent operations
...
-- 5. Create optimized function for dashboard statistics
...
-- 6. Create index for working days calculations
...
-- 7. Add connection pool monitoring table
...
-- Schedule pool stats logging (run this in your cron/job scheduler)
-- SELECT log_connection_pool_stats(); every 5 minutes
