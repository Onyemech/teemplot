...
export default function CompanySetupPage() {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
              ...
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 bg-secondary rounded-lg flex items-center justify-center">
                  {formData.companyLogo ? (
                    <Image src={URL.createObjectURL(formData.companyLogo)} alt="Logo" width={80} height={80} className="rounded-lg" />
                  ) : (
      ...
      case 'payment':
        return <PaymentStep companySize={formData.companySize} onComplete={() => router.push('/dashboard')} />
